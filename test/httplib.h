...
inline unsigned char to_lower(int c) {
        const static unsigned char table[256] = {
            ...
            182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 224, 225, 226,
            227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239,
            240, 241, 242, 243, 244, 245, 246, 215, 248, 249, 250, 251, 252,
            253, 254, 223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233,
...
class ContentReader {
       public:
        ...
        using FormDataReader = std::function<bool(FormDataHeader header,
...
struct Request {
        ...
        std::string get_header_value(const std::string& key,
                                     const char* def = "", size_t id = 0) const;
        ...
        std::string get_trailer_value(const std::string& key,
                                      size_t id = 0) const;
...
struct Response {
        ...
        void set_content(const char* s, size_t n,
...
class PathParamsMatcher final : public MatcherBase {
       ...
       private:
        ...
        // Does not need to handle query parameters as they are parsed before
        // path matching
        static constexpr char separator = '/';

        // Contains static path fragments to match against, excluding the '/'
        // after path params Fragments are separated by path params
...
class Server {
       public:
        ...
        Server& set_error_handler(ErrorHandlerFunc&& handler) {
                return set_error_handler_core(
                    ...
                    std::is_convertible<ErrorHandlerFunc,
                                        HandlerWithResponse>{});
        }
       ...
       private:
        using Handlers = std::vector<
            std::pair<std::unique_ptr<detail::MatcherBase>, Handler>>;
        ...
        Server& set_error_handler_core(HandlerWithResponse handler,
                                       std::true_type);
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
        Result(std::unique_ptr<Response>&& res, Error err,
               Headers&& request_headers, int ssl_error)
            : res_(std::move(res)),
              err_(err),
              request_headers_(std::move(request_headers)),
...
class SSLServer : public Server {
       ...
       private:
        ...
        STACK_OF(X509_NAME) *
            extract_ca_names_from_x509_store(X509_STORE* store);
...
class SSLClient final : public ClientImpl {
       public:
        ...
        explicit SSLClient(
            const std::string& host, int port, X509* client_cert,
            EVP_PKEY* client_key,
            const std::string& private_key_password = std::string());
...
inline bool set_socket_opt(socket_t sock, int level, int optname, int optval) {
        return set_socket_opt_impl(sock, level, optname, &optval,
                                   sizeof(optval));
}
...
inline std::string get_bearer_token_auth(const Request& req) {
        if (req.has_header("Authorization")) {
                constexpr auto bearer_header_prefix_len =
                    detail::str_len("Bearer ");
...
class Result {
       public:
        ...
        // Check if the result is valid (connection succeeded and response
        // received)
        ...
        const Headers& headers() const {
                ...
                return handle_.response ? handle_.response->headers
                                        : empty_headers;
        }

        std::string get_header_value(const std::string& key,
                                     const char* def = "") const {
                return handle_.response
                           ? handle_.response->get_header_value(key, def)
                           : def;
        }

        bool has_header(const std::string& key) const {
                return handle_.response ? handle_.response->has_header(key)
                                        : false;
        }
        ...
        // Call next() to read the next chunk, then access data via
        // data()/size() Returns true if data was read, false when stream is
        // exhausted
        bool next() {
...
inline Result Delete(ClientType& cli, const std::string& path,
                     ...
                     const std::string& body, const std::string& content_type,
                     size_t chunk_size = 8192) {
        return Result{cli.open_stream("DELETE", path, params, headers, body,
                                      content_type),
                      chunk_size};
}
...
class SSEClient {
       ...
       private:
        ...
        bool parse_sse_line(const std::string& line, SSEMessage& msg,
                            int& retry_ms) {
                ...
                if (colon_pos == std::string::npos) {
                        // Line with no colon is treated as field name with
                        // empty value
                        return false;
                }
        ...
        void run_event_loop() {
                ...
                while (running_.load()) {
                        ...
                        if (!last_event_id_.empty()) {
                                request_headers.emplace("Last-Event-ID",
                                                        last_event_id_);
                        }
                        ...
                        if (result.status() != 200) {
                                ...
                                if (result.status() ==
                                        204 ||  // No Content - server wants us
                                                // to stop
                        ...
                        while (running_.load() && result.next()) {
                                ...
                                while ((newline_pos =
                                            buffer.find('\n', line_start)) !=
                                       std::string::npos) {
                                        auto line = buffer.substr(
                                            line_start,
                                            newline_pos - line_start);
                                        line_start = newline_pos + 1;

                                        // Parse the line and check if event is
                                        // complete
                                        ...
                                        if (event_complete &&
                                            !current_msg.data.empty()) {
                                                // Update last_event_id for
                                                // reconnection
...
#endif
        char buf[64];
        if (strftime(buf, sizeof(buf), "%a, %d %b %Y %H:%M:%S GMT", &tm_buf) ==
            0) {
                return std::string();
        }
...
inline time_t parse_http_date(const std::string& date_str) {
        ...
        // RFC 9110 preferred format (HTTP-date): "Sun, 06 Nov 1994 08:49:37
        // GMT"
...
inline bool is_strong_etag(const std::string& s) {
        // Check if the string is a strong ETag (starts and ends with '"', at
        // least 2 chars)
...
inline bool parse_trailers(stream_line_reader& line_reader, Headers& dest,
                           const Headers& src_headers) {
        // NOTE: In RFC 9112, '7.1 Chunked Transfer Coding' mentions "The
        // chunked transfer coding is complete when a chunk with a chunk-size of
        // zero is received, possibly followed by a trailer section, and finally
        ...
        // doesn't care for the existence of the final CRLF. In other words, it
        // seems to be ok whether the final CRLF exists or not in the chunked
        // data. https://www.rfc-editor.org/rfc/rfc9112.html#section-7.1.3
        //
        // According to the reference code in RFC 9112, cpp-httplib now allows
        // chunked transfer coding data without the final CRLF.

        // RFC 7230 Section 4.1.2 - Headers prohibited in trailers
        thread_local case_ignore::unordered_set<std::string>
        ...
        while (strcmp(line_reader.ptr(), "\r\n") != 0) {
                ...
                auto line_end = line_reader.ptr() + line_reader.size() -
                                line_terminator_len;
...
inline void split(const char* b, const char* e, char d,
                  std::function<void(const char*, const char*)> fn) {
        return split(b, e, d, (std::numeric_limits<size_t>::max)(),
                     std::move(fn));
}
...
inline void stream_line_reader::append(char c) {
        if (fixed_buffer_used_size_ < fixed_buffer_size_ - 1) {
                ...
                if (growable_buffer_.empty()) {
                        ...
                        growable_buffer_.assign(fixed_buffer_,
...
inline ssize_t select_impl(socket_t sock, time_t sec, time_t usec) {
        ...
        return handle_EINTR([&]() {
                return select(static_cast<int>(sock + 1), rfds, wfds, nullptr,
                              &tv);
        });
#else
        struct pollfd pfd;
...
inline Error wait_until_socket_is_ready(socket_t sock, time_t sec,
        ...
        auto ret = handle_EINTR([&]() {
                return select(static_cast<int>(sock + 1), &fdsr, &fdsw, nullptr,
                              &tv);
        });
...
inline int getaddrinfo_with_timeout(const char* node, const char* service,
...
#ifdef _WIN32
        // Windows-specific implementation using GetAddrInfoEx with overlapped
        // I/O
        ...
        CFHostRef host_ref =
            CFHostCreateWithName(kCFAllocatorDefault, hostname_ref);
        ...
        auto callback = [](CFHostRef theHost, CFHostInfoType /*typeInfo*/,
                ...
                if (error && error->error != 0) {
                        ...
                        ctx->addresses =
                            CFHostGetAddressing(theHost, &hasBeenResolved);
        ...
        if (!CFHostStartInfoResolution(host_ref, kCFHostAddresses,
                                       &stream_error)) {
                CFHostUnscheduleFromRunLoop(host_ref, run_loop,
        ...
        for (CFIndex i = 0; i < count; i++) {
                CFDataRef addr_data = static_cast<CFDataRef>(
                ...
                socklen_t sockaddr_len =
                    static_cast<socklen_t>(CFDataGetLength(addr_data));

                // Allocate addrinfo structure
                *current = static_cast<struct addrinfo*>(
                ...
                (*current)->ai_socktype =
                    hints ? hints->ai_socktype : SOCK_STREAM;
                (*current)->ai_protocol =
                    hints ? hints->ai_protocol : IPPROTO_TCP;
                (*current)->ai_addrlen = sockaddr_len;

                // Copy sockaddr
                (*current)->ai_addr =
                    static_cast<struct sockaddr*>(malloc(sockaddr_len));
                if (!(*current)->ai_addr) {
                        freeaddrinfo(result_addrinfo);
                        CFRelease(context.addresses);
                        CFRelease(host_ref);
                ...
                if (service && strlen(service) > 0) {
                        ...
                        if (port > 0) {
                                ...
                                } else if (sockaddr_ptr->sa_family ==
                                           AF_INET6) {
    ...
    (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 2))
        // Linux implementation using getaddrinfo_a for asynchronous DNS
        // resolution
...
#else
        // Fallback implementation using thread-based timeout for other Unix
        // systems
        ...
        std::thread resolve_thread([state]() {
                auto thread_result =
                    getaddrinfo(state->node.c_str(), state->service.c_str(),
...
socket_t create_socket(const std::string& host, const std::string& ip, int port,
        ...
        if (hints.ai_family == AF_UNIX) {
...
#ifdef SOCK_CLOEXEC
                auto sock =
                    socket(hints.ai_family, hints.ai_socktype | SOCK_CLOEXEC,
                           hints.ai_protocol);
#else
                ...
                if (sock != INVALID_SOCKET) {
                        ...
                        auto unescaped_host =
                            unescape_abstract_namespace_unix_domain(host);
...
#ifdef _WIN32
                        // Setting SO_REUSEADDR seems not to work well with
                        // AF_UNIX on windows, so remove the option.
                        detail::set_socket_opt(sock, SOL_SOCKET, SO_REUSEADDR,
                                               0);
#endif
        ...
        for (auto rp = result; rp; rp = rp->ai_next) {
                ...
                auto sock = WSASocketW(
                    rp->ai_family, rp->ai_socktype, rp->ai_protocol, nullptr, 0,
...
#ifdef SOCK_CLOEXEC
                auto sock =
                    socket(rp->ai_family, rp->ai_socktype | SOCK_CLOEXEC,
                           rp->ai_protocol);
#else
                auto sock =
...
inline bool get_ip_and_port(const struct sockaddr_storage& addr,
        ...
        if (addr.ss_family == AF_INET) {
                ...
                port = ntohs(reinterpret_cast<const struct sockaddr_in6*>(&addr)
                                 ->sin6_port);
        } else {
                return false;
        }
        ...
        if (getnameinfo(reinterpret_cast<const struct sockaddr*>(&addr),
                        addr_len, ipstr.data(),
                        static_cast<socklen_t>(ipstr.size()), nullptr, 0,
                        NI_NUMERICHOST)) {
                return false;
        }
...
inline void get_remote_ip_and_port(socket_t sock, std::string& ip, int& port) {
        ...
        if (!getpeername(sock, reinterpret_cast<struct sockaddr*>(&addr),
                ...
                if (addr.ss_family == AF_UNIX) {
                        ...
                        if (getsockopt(sock, SOL_SOCKET, SO_PEERCRED, &ucred,
                                       &len) == 0) {
                                port = ucred.pid;
                        }
                        ...
                        if (getsockopt(sock, SOL_LOCAL, SO_PEERPID, &pid,
                                       &len) == 0) {
                                port = pid;
                        }
#endif
...
inline bool brotli_compressor::compress(const char* data, size_t data_length,
        ...
        for (;;) {
                ...
                if (!BrotliEncoderCompressStream(
                        state_, operation, &available_in, &next_in,
...
inline bool zstd_compressor::compress(const char* data, size_t data_length,
        ...
        bool finished;
        do {
                ZSTD_outBuffer output = {buff.data(),
                                         CPPHTTPLIB_COMPRESSION_BUFSIZ, 0};
...
inline bool zstd_decompressor::decompress(const char* data, size_t data_length,
        ...
        while (input.pos < input.size) {
                ZSTD_outBuffer output = {buff.data(),
                                         CPPHTTPLIB_COMPRESSION_BUFSIZ, 0};
                size_t const remaining =
                    ZSTD_decompressStream(ctx_, &output, &input);
...
inline const char* get_header_value(const Headers& headers,
        ...
        if (is_prohibited_header_name(key)) {
                ...
                std::string msg =
                    "Prohibited header name '" + key + "' is specified.";
...
inline ReadContentResult read_content_chunked(Stream& strm, T& x,
        ...
        for (;;) {
                ...
                auto n = dec.read_payload(buf, sizeof(buf), chunk_offset,
                                          chunk_total);
                if (n < 0) {
                        return ReadContentResult::Error;
                }
                ...
                if (!out(buf, static_cast<size_t>(n), chunk_offset,
                         chunk_total)) {
                        return ReadContentResult::Error;
                }
...
bool prepare_content_receiver(T& x, int& status,
        ...
        if (decompress) {
                ...
                if (!encoding.empty()) {
                        ...
                        if (!decompressor) {
                                // Unsupported encoding or no support compiled
                                // in
        ...
        return prepare_content_receiver(
            ...
            [&](const ContentReceiverWithProgress& out) {
                    ...
                    if (is_chunked_transfer_encoding(x.headers)) {
                            auto result = read_content_chunked(
                                strm, x, payload_max_length, out);
                            if (result == ReadContentResult::Success) {
                                    ret = true;
                            } else if (result ==
                                       ReadContentResult::PayloadTooLarge) {
                            ...
                            auto result = read_content_without_length(
                                strm, payload_max_length, out);
                            if (result == ReadContentResult::Success) {
                                    ret = true;
                            } else if (result ==
                                       ReadContentResult::PayloadTooLarge) {
...
inline bool write_content_with_progress(Stream& strm,
                        ...
                        if (write_data(strm, d, l)) {
                                ...
                                if (upload_progress && length > 0) {
                                        size_t current_written =
                                            offset - start_offset;
                                        if (!upload_progress(current_written,
                                                             length)) {
                                                ok = false;
                                                return false;
                                        }
        ...
        while (offset < end_offset && !is_shutting_down()) {
                if (!strm.wait_writable()) {
                        error = Error::Write;
                        return false;
                } else if (!content_provider(offset, end_offset - offset,
                                             data_sink)) {
...
inline bool write_content(Stream& strm, const ContentProvider& content_provider,
                          ...
                          Error& error) {
        return write_content_with_progress<T>(strm, content_provider, offset,
                                              length, is_shutting_down, nullptr,
                                              error);
}
        ...
        data_sink.write = [&](const char* d, size_t l) -> bool {
                if (ok) {
                        ...
                        if (compressor.compress(
                                ...
                                if (!payload.empty()) {
                                        // Emit chunked response header and
                                        // footer for each chunk
                                        auto chunk =
                                            from_i_to_hex(payload.size()) +
                                            "\r\n" + payload + "\r\n";
                                        if (!write_data(strm, chunk.data(),
                                                        chunk.size())) {
                                                ok = false;
                                        }
                                }
                ...
                if (trailer) {
                        for (const auto& kv : *trailer) {
                                std::string field_line =
                                    kv.first + ": " + kv.second + "\r\n";
                                if (!write_data(strm, field_line.data(),
                                                field_line.size())) {
                                        ok = false;
                                }
                        }
...
inline bool parse_accept_header(const std::string& s,
        ...
        // Check for invalid patterns: leading/trailing commas or consecutive
        // commas
        if (s.front() == ',' || s.back() == ',' ||
            ...
            [&](const char* b, const char* e) {
                    ...
                    if (q_pos != std::string::npos) {
                            ...
                            accept_entry.media_type =
                                trim_copy(entry.substr(0, q_pos));
                            ...
                            std::string quality_str = trim_copy(
                                entry.substr(q_start, q_end - q_start));
...

                            {
                                    ...
                                    auto res = detail::from_chars(
                                        quality_str.data(),
                                        quality_str.data() + quality_str.size(),
                                        v);
                                    if (res.ec == std::errc{}) {
                            ...
                            if (accept_entry.quality < 0.0 ||
                                accept_entry.quality > 1.0) {
                                    has_invalid_entry = true;
                                    return;
                            }
                    } else {
                            // No quality parameter, use entire entry as media
                            // type
                    ...
                    if (param_pos != std::string::npos) {
                            accept_entry.media_type = trim_copy(
                                accept_entry.media_type.substr(0, param_pos));
                    }
                    ...
                    // Check for basic media type format (should contain '/' or
                    // be '*')
                    if (accept_entry.media_type != "*" &&
                        accept_entry.media_type.find('/') ==
                            std::string::npos) {
                            has_invalid_entry = true;
                            return;
                    }
        ...
        std::sort(
            ...
            [](const AcceptEntry& a, const AcceptEntry& b) {
                    if (a.quality != b.quality) {
                            return a.quality >
                                   b.quality;  // Higher quality first
                    }
                    return a.order <
                           b.order;  // Earlier order first for same quality
            });
                ...
                while (buf_size() > 0) {
                        switch (state_) {
                                case 0: {  // Initial boundary
                                        auto pos =
                                            buf_find(dash_boundary_crlf_);
                                        if (pos == buf_size()) {
                                                return true;
                                        }
                                        buf_erase(pos +
                                                  dash_boundary_crlf_.size());
                                        state_ = 1;
                                        break;
                                }
                                case 1: {  // New entry
                                ...
                                case 2: {  // Headers
                                        ...
                                        while (pos < buf_size()) {
                                                ...
                                                // Parse and emplace space
                                                // trimmed headers into a map
                                                if (!parse_header(
                                                        header.data(),
                                                        header.data() +
                                                            header.size(),
                                                ...
                                                if (start_with_case_ignore(
                                                        header,
                                                        header_content_type)) {
                                                        file_
                                                            .content_type = trim_copy(
                                                            header.substr(str_len(
                                                                header_content_type)));
                                                } else {
                                                        ...
                                                        if (std::regex_match(
                                                                ...
                                                                if (it !=
                                                                    params
                                                                        ...
                                                                        // UTF-8
                                                                        // encoding...
                                                                        thread_local const std::
                                                                            regex re_rfc5987_encoding(
                                                                                R"~(^UTF-8''(.+?)$)~",
                                                                                std::regex_constants::
                                                                                    icase);
                                ...
                                case 3: {  // Body
                                        if (crlf_dash_boundary_.size() >
                                            buf_size()) {
                                                return true;
                                        }
                                        auto pos =
                                            buf_find(crlf_dash_boundary_);
                                        if (pos < buf_size()) {
                                                if (!content_callback(
                                                        buf_data(), pos)) {
                                                        is_valid_ = false;
       ...
       private:
        ...
        bool start_with_case_ignore(const std::string& a, const char* b) const {
                ...
                for (size_t i = 0; i < b_len; i++) {
                        if (case_ignore::to_lower(a[i]) !=
                            case_ignore::to_lower(b[i])) {
                                return false;
                        }
                }
                return true;
...
inline std::string random_string(size_t length) {
        ...
        thread_local auto engine([]() {
                ...
                // library, doing better requires either some ugly hacks or
...
inline void coalesce_ranges(Ranges& ranges, size_t content_length) {
        ...
        for (auto& r : ranges) {
                ...
                if (first_pos == -1) {
                        first_pos =
                            static_cast<ssize_t>(content_length) - last_pos;
                ...
                // Coalesce with previous range if overlapping or adjacent (but
                // not identical)
                if (!coalesced.empty()) {
                        auto& prev = coalesced.back();
                        // Check if current range overlaps or is adjacent to
                        // previous range but don't coalesce identical ranges
                        // (allow duplicates)
                        if (first_pos <= prev.second + 1 &&
                            !(first_pos == prev.first &&
                              last_pos == prev.second)) {
...
inline bool range_error(Request& req, Response& res) {
        if (!req.ranges.empty() && 200 <= res.status && res.status < 300) {
                ssize_t content_len = static_cast<ssize_t>(
                    res.content_length_ ? res.content_length_
                                        : res.body.size());

                std::vector<std::pair<ssize_t, ssize_t>> processed_ranges;
                size_t overwrapping_count = 0;

                // NOTE: The following Range check is based on '14.2. Range' in
                // RFC 9110 'HTTP Semantics' to avoid potential
                ...
                for (auto& r : req.ranges) {
                        ...
                        // A client can limit the number of bytes requested
                        // without knowing the size of the selected
                        // representation. If the last-pos value is absent, or
                        // if the value is greater than or equal to the current
                        // length of the representation data, the byte range is
                        // interpreted as the remainder of the representation
                        // (i.e., the server replaces the value of last-pos with
                        // a value that is one less than the current length of
                        ...
                        // Request must not have more than two overlapping
                        // ranges
...
bool process_multipart_ranges_data(const Request& req,
        ...
        for (size_t i = 0; i < req.ranges.size(); i++) {
                ...
                if (!content(offset_and_length.first,
...
inline bool write_multipart_ranges_data(Stream& strm, const Request& req,
        ...
        return process_multipart_ranges_data(
            ...
            [&](const std::string& token) { strm.write(token); },
            [&](size_t offset, size_t length) {
                    return write_content(strm, res.content_provider_, offset,
...
inline bool retrieve_certs_from_keychain(CFObjectPtr<CFArrayRef>& certs) {
        ...
        if (SecItemCopyMatching(query.get(), &security_items) !=
                errSecSuccess ||
...
inline bool retrieve_root_certs_from_keychain(CFObjectPtr<CFArrayRef>& certs) {
        ...
        if (SecTrustCopyAnchorCertificates(&root_security_items) !=
            errSecSuccess) {
                return false;
        }
...
inline bool add_certs_to_x509_store(CFArrayRef certs, X509_STORE* store) {
        ...
        for (auto i = 0; i < CFArrayGetCount(certs); ++i) {
                ...
                if (SecItemExport(cert, kSecFormatX509Cert, 0, nullptr,
                                  &cert_data) != errSecSuccess) {
                        continue;
                }

                CFObjectPtr<CFDataRef> cert_data_ptr(cert_data,
                                                     cf_object_ptr_deleter);

                auto encoded_cert = static_cast<const unsigned char*>(
                    CFDataGetBytePtr(cert_data_ptr.get()));

                auto x509 = d2i_X509(NULL, &encoded_cert,
                                     CFDataGetLength(cert_data_ptr.get()));
        ...
        if (res.has_header(auth_key)) {
                ...
                if (pos != std::string::npos) {
                        ...
                        if (type == "Basic") {
                                ...
                                s = s.substr(pos + 1);
                                auto beg = std::sregex_iterator(s.begin(),
                                                                s.end(), re);
                                for (auto i = beg; i != std::sregex_iterator();
                                     ++i) {
                                        const auto& m = *i;
                                        auto key = s.substr(
...
inline std::string encode_path_component(const std::string& component) {
        ...
        for (size_t i = 0; i < component.size(); i++) {
                ...
                // Unreserved characters per RFC 3986: ALPHA / DIGIT / "-" / "."
                // / "_" / "~"
        ...
        for (size_t i = 0; i < component.size(); i++) {
                if (component[i] == '%' && i + 1 < component.size()) {
                        if (component[i + 1] == 'u') {
                                ...
                                auto val = 0;
                                if (detail::from_hex_to_i(component, i + 2, 4,
                                                          val)) {
                                ...
                                auto val = 0;
                                if (detail::from_hex_to_i(component, i + 1, 2,
                                                          val)) {
                                        // 2 digits hex codes
                                        result += static_cast<char>(val);
                                        i += 2;  // 'XX'
...
inline std::string encode_query_component(const std::string& component,
        ...
        for (size_t i = 0; i < component.size(); i++) {
                ...
                // Query-safe sub-delimiters (excluding & and = which are query
                // delimiters)
                else if (c == '!' || c == '$' || c == '\'' || c == '(' ||
                         c == ')' || c == '*' || c == ',' || c == ';') {
                        result += static_cast<char>(c);
...
inline ssize_t detail::BodyReader::read(char* buf, size_t len) {
        ...
        if (n == 0) {
                // Final chunk observed. Leave trailer parsing to the caller
                // (StreamHandle).
                eof = true;
                return 0;
        }
...
inline bool SocketStream::wait_readable() const {
        if (max_timeout_msec_ <= 0) {
                return select_read(sock_, read_timeout_sec_,
...
inline ssize_t SocketStream::read(char* ptr, size_t size) {
...
#else
        size = (std::min)(size, static_cast<size_t>(
        ...
        if (read_buff_off_ < read_buff_content_size_) {
                ...
                if (size <= remaining_size) {
                        ...
                        memcpy(ptr, read_buff_.data() + read_buff_off_,
                               remaining_size);
    ...
    : MatcherBase(pattern) {
        ...
        while (true) {
                ...
                static_fragments_.push_back(pattern.substr(
                    last_param_end, marker_pos - last_param_end + 1));
                ...
                auto param_name = pattern.substr(param_name_start,
                                                 sep_pos - param_name_start);
...
inline bool PathParamsMatcher::match(Request& request) const {
        ...
        for (size_t i = 0; i < static_fragments_.size(); ++i) {
                ...
                // Avoid unnecessary allocation by using strncmp instead of
                // substr + comparison
                if (std::strncmp(request.path.c_str() + starting_pos,
                                 fragment.c_str(), fragment.length()) != 0) {
                        return false;
                }

                starting_pos += fragment.length();

                // Should only happen when we have a static fragment after a
                // param Example: '/users/:id/subscriptions' The 'subscriptions'
                // fragment here does not have a corresponding param
...
inline Server& Server::Delete(const std::string& pattern, Handler handler) {
        delete_handlers_.emplace_back(make_matcher(pattern),
                                      std::move(handler));
        return *this;
}
...
inline Server& Server::Options(const std::string& pattern, Handler handler) {
        options_handlers_.emplace_back(make_matcher(pattern),
                                       std::move(handler));
        return *this;
}
        ...
        if (stat.is_dir()) {
                ...
                if (!mnt.empty() && mnt[0] == '/') {
                        base_dirs_.push_back(
                            {std::move(mnt), dir, std::move(headers)});
                        return true;
                }
        }
        return false;
...
inline bool Server::parse_request_line(const char* s, Request& req) const {
...

        {
                ...
                detail::split(
                    s, s + len, ' ', [&](const char* b, const char* e) {
                            switch (count) {
                                    case 0:
...

        {
                ...
                detail::divide(req.target, '?',
                               [&](const char* lhs_data, std::size_t lhs_size,
                                   const char* rhs_data, std::size_t rhs_size) {
                                       req.path = decode_path_component(
                                           std::string(lhs_data, lhs_size));
                                       detail::parse_query_text(
                                           rhs_data, rhs_size, req.params);
                               });
        }
...
inline bool Server::write_response_core(Stream& strm, bool close_connection,
        ...
        if (res.body.empty() && !res.content_length_ &&
            !res.content_provider_ && !res.has_header("Content-Length")) {
        ...
        if (req.method != "HEAD") {
                if (!res.body.empty()) {
                        if (!detail::write_data(strm, res.body.data(),
                                                res.body.size())) {
                                ret = false;
                        }
    ...
    const std::string& boundary, const std::string& content_type) {
        ...
        if (res.content_length_ > 0) {
                if (req.ranges.empty()) {
                        ...
                        auto offset_and_length =
                            detail::get_range_offset_and_length(
                                req.ranges[0], res.content_length_);

                        return detail::write_content(
                            strm, res.content_provider_,
                            offset_and_length.first, offset_and_length.second,
                            is_shutting_down);
                } else {
                ...
                if (res.is_chunked_content_provider_) {
                        ...
                        if (type == detail::EncodingType::Gzip) {
                                ...
                                compressor = detail::make_unique<
                                    detail::gzip_compressor>();
#endif
                        } else if (type == detail::EncodingType::Brotli) {
#ifdef CPPHTTPLIB_BROTLI_SUPPORT
                                compressor = detail::make_unique<
                                    detail::brotli_compressor>();
#endif
                        } else if (type == detail::EncodingType::Zstd) {
#ifdef CPPHTTPLIB_ZSTD_SUPPORT
                                compressor = detail::make_unique<
                                    detail::zstd_compressor>();
#endif
                        } else {
                                compressor =
                                    detail::make_unique<detail::nocompressor>();
                        }
...
inline bool Server::read_content(Stream& strm, Request& req, Response& res) {
        ...
        if (read_content_core(
                ...
                [&](const char* buf, size_t n) {
                        ...
                        // Avoid computing (req.body.size() + n) directly
                        // because adding two unsigned `size_t` values can wrap
                        // around and produce a small result instead of
                        // indicating overflow. Instead, check using
                        // subtraction: ensure `n` does not exceed the remaining
                        // capacity `max_size() - size()`.
                        if (req.body.size() >= req.body.max_size() ||
                            n > req.body.max_size() - req.body.size()) {
                                return false;
                        }

                        // Limit decompressed body size to payload_max_length_
                        // to protect against "zip bomb" attacks where a small
                        // compressed payload decompresses to a massive size.
                ...
                [&](const FormData& file) {
                        if (count++ ==
                            CPPHTTPLIB_MULTIPART_FORM_DATA_FILE_MAX_COUNT) {
                                output_error_log(Error::TooManyFormDataFiles,
                                                 &req);
                                return false;
                        }
                        ...
                        if (file.filename.empty()) {
                                cur_field = req.form.fields.emplace(
                                    file.name,
                                    FormField{file.name, file.content,
                                              file.headers});
                                is_text_field = true;
                        } else {
                                cur_file =
                                    req.form.files.emplace(file.name, file);
                                is_text_field = false;
                        }
                        return true;
                },
                ...
                if (!content_type.find("application/x-www-form-urlencoded")) {
                        if (req.body.size() >
                            CPPHTTPLIB_FORM_URL_ENCODED_PAYLOAD_MAX_LENGTH) {
                                res.status =
                                    StatusCode::PayloadTooLarge_413;  // NOTE:
                                                                      // should
                                                                      // be 414?
                                output_error_log(Error::ExceedMaxPayloadSize,
                                                 &req);
                                return false;
                        }
...
inline bool Server::read_content_core(
    ...
    FormDataHeader multipart_header, ContentReceiver multipart_receiver) const {
        ...
        if (req.is_multipart_form_data()) {
                ...
                out = [&](const char* buf, size_t n, size_t /*off*/,
                          size_t /*len*/) {
        ...
        // RFC 7230 Section 3.3.3: If this is a request message and none of the
        // above are true (no Transfer-Encoding and no Content-Length), then the
        // message body length is zero (no message body is present).
        //
        // For non-SSL builds, peek into the socket to detect clients that send
        // a body without a Content-Length header (raw HTTP over TCP). If there
        // is pending data that exceeds the configured payload limit, treat this
        // as an oversized request and fail early (causing connection close).
        // For SSL builds we cannot reliably peek the decrypted application
        // bytes, so keep the original behaviour.
...
inline bool Server::handle_file_request(Request& req, Response& res) {
        for (const auto& entry : base_dirs_) {
                // Prefix match
                if (!req.path.compare(0, entry.mount_point.size(),
                                      entry.mount_point)) {
                        std::string sub_path =
                        ...
                        if (detail::is_valid_path(sub_path)) {
                                ...
                                if (stat.is_file()) {
                                        ...
                                        auto last_modified =
                                            detail::file_mtime_to_http_date(
                                                mtime);
...
inline bool Server::check_if_not_modified(const Request& req, Response& res,
        ...
        if (req.has_header("If-None-Match")) {
                if (!etag.empty()) {
                        ...
                        // NOTE: We use exact string matching here. This works
                        // correctly because our server always generates weak
                        // ETags (W/"..."), and clients typically send back the
                        // same ETag they received. RFC 9110 Section 8.8.3.2
                        // allows weak comparison for If-None-Match, where W/"x"
                        // and "x" would match, but this simplified
                        // implementation requires exact matches.
        ...
        if (!req.ranges.empty() && req.has_header("If-Range")) {
                ...
                auto is_valid_range = [&]() {
                        if (detail::is_strong_etag(val)) {
                                // RFC 9110 Section 13.1.5: If-Range requires
                                // strong ETag comparison.
                                return (!etag.empty() && val == etag);
                        } else if (detail::is_weak_etag(val)) {
                                // Weak ETags are not valid for If-Range (RFC
                                // 9110 Section 13.1.5)
                                return false;
                        } else {
                ...
                if (!is_valid_range()) {
                        // Validator doesn't match: ignore Range and serve full
                        // content
                        req.ranges.clear();
                        return false;
                }
...
inline socket_t Server::create_server_socket(
    ...
    SocketOptions socket_options) const {
        return detail::create_socket(
            host, std::string(), port, address_family_, socket_flags,
...
inline int Server::bind_internal(const std::string& host, int port,
        ...
        if (port == 0) {
                ...
                if (addr.ss_family == AF_INET) {
                        ...
                        output_error_log(Error::UnsupportedAddressFamily,
                                         nullptr);
                        return -1;
                }
        } else {
                ...
                while (svr_sock_ != INVALID_SOCKET) {
...
#if defined _WIN32
                        // sockets connected via WASAccept inherit flags
                        // NO_HANDLE_INHERIT, OVERLAPPED
                        socket_t sock =
                            WSAAccept(svr_sock_, nullptr, nullptr, nullptr, 0);
#elif defined SOCK_CLOEXEC
                        socket_t sock =
                            accept4(svr_sock_, nullptr, nullptr, SOCK_CLOEXEC);
#else
                        ...
                        if (sock == INVALID_SOCKET) {
                                if (errno == EMFILE) {
                                        ...
                                        // descriptors has been reached. Try to
                                        // accept new connections after a short
                                        // sleep.
                                        std::this_thread::sleep_for(
                                            std::chrono::microseconds{1});
                                        continue;
                                ...
                                if (svr_sock_ != INVALID_SOCKET) {
                                        ...
                                        ret = false;
                                        output_error_log(Error::Connection,
                                                         nullptr);
                                } else {
                                        ;  // The server socket was closed by
                                           // user.
                                }
                                break;
                        }
                        ...
                        detail::set_socket_opt_time(
                            sock, SOL_SOCKET, SO_RCVTIMEO, read_timeout_sec_,
                            read_timeout_usec_);
                        detail::set_socket_opt_time(
                            sock, SOL_SOCKET, SO_SNDTIMEO, write_timeout_sec_,
                            write_timeout_usec_);

                        if (!task_queue->enqueue([this, sock]() {
                                    process_and_close_socket(sock);
                            })) {
                                output_error_log(Error::ResourceExhaustion,
                                                 nullptr);
                                detail::shutdown_socket(sock);
                                detail::close_socket(sock);
                        }
        ...
        if (res.body.empty()) {
                if (res.content_length_ > 0) {
                        ...
                        if (res.content_provider_) {
                                if (res.is_chunked_content_provider_) {
                                        res.set_header("Transfer-Encoding",
                                                       "chunked");
                                        if (type ==
                                            detail::EncodingType::Gzip) {
                                                res.set_header(
...
inline std::string get_client_ip(
    ...
    const std::vector<std::string>& trusted_proxies) {
        ...
        detail::split(x_forwarded_for.data(),
                      ...
                      [&](const char* b, const char* e) {
                              auto r = detail::trim(b, e, 0,
                                                    static_cast<size_t>(e - b));
                              ip_list.emplace_back(
                                  std::string(b + r.first, b + r.second));
                      });

        for (size_t i = 0; i < ip_list.size(); ++i) {
                auto ip = ip_list[i];

                auto is_trusted_proxy = std::any_of(
                    trusted_proxies.begin(), trusted_proxies.end(),
                    [&](const std::string& proxy) { return ip == proxy; });

                if (is_trusted_proxy) {
                        if (i == 0) {
                                // If the trusted proxy is the first IP, there's
                                // no preceding client IP
                                return ip;
                        } else {
                                // Return the IP immediately before the trusted
                                // proxy
                                return ip_list[i - 1];
                        }
                }
...
inline bool Server::process_request(
    ...
    const std::function<void(Request&)>& setup_request) {
        ...
        if (!trusted_proxies_.empty() && req.has_header("X-Forwarded-For")) {
                ...
                req.remote_addr =
                    get_client_ip(x_forwarded_for, trusted_proxies_);
        } else {
                req.remote_addr = remote_addr;
        }
        req.remote_port = remote_port;
        ...
        if (req.has_header("Accept")) {
                ...
                if (!detail::parse_accept_header(accept_header,
                                                 req.accept_content_types)) {
                        res.status = StatusCode::BadRequest_400;
        ...
        if (req.has_header("Range")) {
                ...
                if (!detail::parse_range_header(range_header_value,
                                                req.ranges)) {
        ...
        if (routed) {
                ...
                return write_response_with_content(strm, close_connection, req,
                                                   res);
        } else {
                if (res.status == -1) {
                        res.status = StatusCode::NotFound_404;
...
inline bool ClientImpl::read_response_line(Stream& strm, const Request& req,
        ...
        // Ignore '100 Continue' (only when not using Expect: 100-continue
        // explicitly)
...
inline bool ClientImpl::send_(Request& req, Response& res, Error& error) {
        {
                ...
                // Set this to false immediately - if it ever gets set to true
                // by the end of the request, we know another thread instructed
                // us to close the socket.
                ...
                if (socket_.is_open()) {
                        ...
                        if (!is_alive) {
                                ...
                                // non-gracefully if it seems like the other
                                // side has already closed the connection Also,
                                // there cannot be any requests in flight from
                                // other threads since we locked request_mutex_,
                                // so safe to close everything immediately
                ...
                // closed by anyone else while this request is ongoing, even
                // though we will be releasing the mutex.
        ...
        auto se = detail::scope_exit([&]() {
                // Briefly lock mutex in order to mark that a request is no
                // longer ongoing
                std::lock_guard<std::mutex> guard(socket_mutex_);
                socket_requests_in_flight_ -= 1;
                if (socket_requests_in_flight_ <= 0) {
...
inline void ClientImpl::prepare_default_headers(Request& r, bool for_stream,
        ...
        if (!r.body.empty()) {
                ...
                if (!r.has_header("Content-Length")) {
                        r.headers.emplace("Content-Length",
    ...
    const std::string& content_type) {
...

        {
                ...
                if (!is_alive) {
                        ...
                        if (is_ssl()) {
                                auto& scli = static_cast<SSLClient&>(*this);
                                if (!proxy_host_.empty() && proxy_port_ != -1) {
                                        if (!scli.initialize_ssl(
                                                socket_, handle.error)) {
                                                handle.response.reset();
                                                return handle;
                                        }
                                }
        ...
        if (is_ssl() && handle.connection_->ssl) {
                handle.socket_stream_ =
                    detail::make_unique<detail::SSLSocketStream>(
                        handle.connection_->sock, handle.connection_->ssl,
                        read_timeout_sec_, read_timeout_usec_,
                        write_timeout_sec_, write_timeout_usec_);
        } else {
                handle.socket_stream_ =
                    detail::make_unique<detail::SocketStream>(
                        handle.connection_->sock, read_timeout_sec_,
        ...
        auto content_encoding =
            handle.response->get_header_value("Content-Encoding");
        if (!content_encoding.empty()) {
                handle.decompressor_ =
                    detail::create_decompressor(content_encoding);
        }
...
inline ssize_t ClientImpl::StreamHandle::read(char* buf, size_t len) {
        ...
        if (n <= 0 && body_reader_.chunked && !trailers_parsed_ && stream_) {
                ...
                if (body_reader_.chunked_decoder) {
                        ...
                        if (!dec.parse_trailers_into(response->trailers,
                                                     response->headers)) {
                                return n;
                        }
                }
...
inline ssize_t ClientImpl::StreamHandle::read_with_decompression(char* buf,
        ...
        if (decompress_offset_ < decompress_buffer_.size()) {
                ...
                std::memcpy(buf, decompress_buffer_.data() + decompress_offset_,
                            to_copy);
        ...
        while (true) {
                auto n = detail::read_body_content(stream_, body_reader_,
                                                   compressed_buf,
...
inline void ClientImpl::StreamHandle::parse_trailers_if_needed() {
        if (!response || !stream_ || !body_reader_.chunked ||
            trailers_parsed_) {
                return;
        }
...
inline bool ClientImpl::handle_request(Stream& strm, Request& req,
        ...
        if (res.get_header_value("Connection") == "close" ||
            (res.version == "HTTP/1.0" &&
             res.reason != "Connection established")) {
                // TODO this requires a not-entirely-obvious chain of calls to
                // be correct for this to be safe.

                // This is safe to call because handle_request is only called by
                // send_ which locks the request mutex during the process. It
                // would be a bug to call it from a different thread since it's
                // a thread-safety issue to do these things to the socket if
                // another thread is using the socket.
                std::lock_guard<std::mutex> guard(socket_mutex_);
                shutdown_ssl(socket_, true);
                shutdown_socket(socket_);
                close_socket(socket_);
        ...
        if ((res.status == StatusCode::Unauthorized_401 ||
            ...
            req.authorization_count_ < 5) {
                auto is_proxy =
                    res.status == StatusCode::ProxyAuthenticationRequired_407;
                const auto& username = is_proxy ? proxy_digest_auth_username_
                                                : digest_auth_username_;
                const auto& password = is_proxy ? proxy_digest_auth_password_
                                                : digest_auth_password_;

                if (!username.empty() && !password.empty()) {
                        std::map<std::string, std::string> auth;
                        if (detail::parse_www_authenticate(res, auth,
                                                           is_proxy)) {
                                Request new_req = req;
                                new_req.authorization_count_ += 1;
                                new_req.headers.erase(
                                ...
                                new_req.headers.insert(
                                    detail::make_digest_authentication_header(
                                        ...
                                        detail::random_string(10), username,
                                        password, is_proxy));
...
inline bool ClientImpl::redirect(Request& req, Response& res, Error& error) {
        ...
        return create_redirect_client(next_scheme, next_host, next_port, req,
                                      res, path, location, error);
}
...
inline bool ClientImpl::create_redirect_client(
    ...
    Error& error) {
        ...
        auto headers_to_remove = std::vector<std::string>{
            "Host", "Proxy-Authorization", "Authorization"};
        ...
        if (need_ssl) {
                ...
                if (!proxy_host_.empty() && proxy_port_ != -1) {
                        // Critical: Disable SSL verification for proxy
                        // environments
                        ...
                        // For direct SSL connections, copy SSL verification
                        // settings
                        redirect_client.enable_server_certificate_verification(
                ...
                if (!ca_cert_file_path_.empty()) {
                        redirect_client.set_ca_cert_path(ca_cert_file_path_,
                                                         ca_cert_dir_path_);
                }

                // Client certificates are set through constructor for SSLClient
                // NOTE: SSLClient constructor already takes client_cert_path
                // and client_key_path so we need to create it properly if
                // client certs are needed

                // Execute the redirect
                return detail::redirect(redirect_client, req, res, path,
                                        location, error);
#else
...
inline void ClientImpl::setup_redirect_client(ClientType& client) {
        ...
        if (!basic_auth_username_.empty()) {
                client.set_basic_auth(basic_auth_username_,
        ...
        if (!digest_auth_username_.empty()) {
                client.set_digest_auth(digest_auth_username_,
                                       digest_auth_password_);
        }
#endif
        ...
        // NOTE: DO NOT copy default_headers_ as they may contain stale Host
        // headers Each new client should generate its own headers based on its
        // target host
}
...
inline bool ClientImpl::write_content_with_provider(Stream& strm,
        ...
        if (req.is_chunked_content_provider_) {
                ...
                if (compress_) {
                        compressor =
                            detail::make_unique<detail::gzip_compressor>();
                } else
#endif
                {
                        compressor =
                ...
                return detail::write_content_chunked(
                    strm, req.content_provider_, is_shutting_down, *compressor,
                    error);
        } else {
                return detail::write_content_with_progress(
                    strm, req.content_provider_, 0, req.content_length_,
...
inline bool ClientImpl::write_request(Stream& strm, Request& req,
        ...
        if (req.body.empty()) {
                if (req.content_provider_) {
                        if (!req.is_chunked_content_provider_) {
                                if (!req.has_header("Content-Length")) {
                                        auto length =
                                            std::to_string(req.content_length_);
                                        req.set_header("Content-Length",
                                                       length);
                                }
                        }
                } else {
        ...
        // Request line and headers
        {
                ...
                // Encode path part. If the original `req.path` already
                // contained a query component, preserve its raw query string
                // (including parameter order) instead of reparsing and
                // reassembling it which may reorder parameters due to container
                // ordering (e.g. `Params` uses `std::multimap`). When there is
                // no query in `req.path`, fall back to building a query from
                // `req.params` so existing callers that pass `Params` continue
                // to work.
                auto path_with_query =
                    path_encode_ ? detail::encode_path(path_part) : path_part;

                if (!query_part.empty()) {
                        // Normalize the query string (decode then re-encode)
                        // while preserving the original parameter order.
                        auto normalized =
                            detail::normalize_query_string(query_part);
                        if (!normalized.empty()) {
                                path_with_query += '?' + normalized;
                        }

                        // Still populate req.params for handlers/users who read
                        // them.
                        detail::parse_query_text(query_part, req.params);
                } else {
                        // No query in path; parse any query_part (empty) and
                        // append params from `req.params` when present
                        // (preserves prior behavior for callers who provide
                        // Params separately).
                        detail::parse_query_text(query_part, req.params);
                        if (!req.params.empty()) {
                                path_with_query = append_query_params(
                                    path_with_query, req.params);
                        }
                }

                // Write request line and headers
                detail::write_request_line(bstrm, req.method, path_with_query);
                if (!detail::check_and_write_headers(bstrm, req.headers,
                                                     header_writer_, error)) {
                        output_error_log(error, &req);
                        return false;
                }
        ...
        // After sending request line and headers, wait briefly for an early
        // server response (e.g. 4xx) and avoid sending a potentially large
        // request body unnecessarily. This workaround is only enabled on
        // Windows because Unix platforms surface write errors (EPIPE) earlier;
        // on Windows kernel send buffering can accept large writes even when
        // the peer already responded. Check the stream first (which covers SSL
        // via `is_readable()`), then fall back to select on the socket. Only
        // perform the wait for very large request bodies to avoid interfering
        // with normal small requests and reduce side-effects. Poll briefly (up
        // to 50ms as default) for an early response. Skip this check when using
        // Expect: 100-continue, as the protocol handles early responses
        // properly.
#if defined(_WIN32)
        if (!skip_body &&
            ...
            req.path.size() > CPPHTTPLIB_REQUEST_URI_MAX_LENGTH) {
                ...
                for (;;) {
                        // Prefer socket-level readiness to avoid SSL_pending()
                        // false-positives from SSL internals. If the underlying
                        // socket is readable, assume an early response may be
                        // present.
                        auto sock = strm.socket();
                        if (sock != INVALID_SOCKET &&
                            detail::select_read(sock, 0, 0) > 0) {
                                return false;
                        }

                        // Fallback to stream-level check for non-socket streams
                        // or when the socket isn't reporting readable. Avoid
                        // using `is_readable()` for SSL, since `SSL_pending()`
                        // may report buffered records that do not indicate a
                        // complete application-level response yet.
...
inline bool ClientImpl::write_request_body(Stream& strm, Request& req,
        ...
        if (req.upload_progress) {
                ...
                while (written < body_size) {
                        size_t to_write = (std::min)(CPPHTTPLIB_SEND_BUFSIZ,
                                                     body_size - written);
                        if (!detail::write_data(strm, data + written,
                                                to_write)) {
                                error = Error::Write;
                                output_error_log(error, &req);
                                return false;
                ...
                if (!detail::write_data(strm, req.body.data(),
                                        req.body.size())) {
                        error = Error::Write;
                        output_error_log(error, &req);
                        return false;
    ...
    Error& error) {
        ...
        if (compress_ && !content_provider_without_length) {
                ...
                if (content_provider) {
                        ...
                        data_sink.write = [&](const char* data,
                                              size_t data_len) -> bool {
                                if (ok) {
                                        auto last =
                                            offset + data_len == content_length;
...
inline bool ClientImpl::process_request(Stream& strm, Request& req,
        ...
        if (CPPHTTPLIB_EXPECT_100_THRESHOLD > 0 && !req.has_header("Expect")) {
                auto body_size =
                    req.body.empty() ? req.content_length_ : req.body.size();
        ...
        auto expect_100_continue =
            req.get_header_value("Expect") == "100-continue";

        // Send request (skip body if using Expect: 100-continue)
        auto write_request_success = write_request(strm, req, close_connection,
                                                   error, expect_100_continue);

#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
        if (is_ssl()) {
                auto is_proxy_enabled =
        ...
        if (expect_100_continue && CPPHTTPLIB_EXPECT_100_TIMEOUT_MSECOND > 0) {
                ...
                time_t usec =
                    (CPPHTTPLIB_EXPECT_100_TIMEOUT_MSECOND % 1000) * 1000;
                auto ret = detail::select_read(strm.socket(), sec, usec);
                if (ret <= 0) {
                        // Timeout or error: send body anyway (server didn't
                        // respond in time)
                        if (!write_request_body(strm, req, error)) {
                                return false;
                        }
                        expect_100_continue =
                            false;  // Switch to normal response handling
                }
        }

        // Receive response and headers
        // When using Expect: 100-continue, don't auto-skip `100 Continue`
        // response
        ...
        if (expect_100_continue) {
                ...
                // If not 100 Continue, server returned an error; proceed with
                // that response
        }
            ...
            req.method != "CONNECT") {
                ...
                if (res.has_header("Content-Length")) {
                        if (!req.content_receiver) {
                                auto len =
                                    res.get_header_value_u64("Content-Length");
                ...
                if (res.status != StatusCode::NotModified_304) {
                        ...
                        if (!detail::read_content(
                                strm, res, (std::numeric_limits<size_t>::max)(),
    ...
    const FormDataProviderItems& provider_items) const {
        ...
        return [&, cur_item, cur_start](size_t offset,
                ...
                if (!offset && !items.empty()) {
                        sink.os << detail::serialize_multipart_formdata(
                            items, boundary, false);
                        return true;
                } else if (cur_item < provider_items.size()) {
                        if (!cur_start) {
                                const auto& begin = detail::
                        ...
                        if (!provider_items[cur_item].provider(
                                offset - cur_start, cur_sink)) {
                                return false;
                        }
                        ...
                        sink.os << detail::serialize_multipart_formdata_finish(
                            boundary);
                        sink.done();
                        return true;
                }
...
inline Result ClientImpl::Post(const std::string& path, const Headers& headers,
        ...
        const auto& body =
            detail::serialize_multipart_formdata(items, boundary);
        return Post(path, headers, body, content_type, progress);
}
...
inline void ClientImpl::stop() {
        ...
        if (socket_requests_in_flight_ > 0) {
                ...
                // Aside from that, we set a flag for the socket to be closed
                // when we're done.
                socket_should_be_closed_when_request_is_done_ = true;
                return;
        }

        // Otherwise, still holding the mutex, we can shut everything down
        // ourselves
        shutdown_ssl(socket_, true);
        shutdown_socket(socket_);
        close_socket(socket_);
...
inline X509_STORE* ClientImpl::create_ca_cert_store(const char* ca_cert,
        ...
        if (cts) {
                for (auto i = 0; i < static_cast<int>(sk_X509_INFO_num(inf));
                     i++) {
...
inline void ssl_delete(std::mutex& ctx_mutex, SSL* ssl, socket_t sock,
        ...
        // Note that it is not always possible to avoid SIGPIPE, this is merely
        // a best-efforts.
        if (shutdown_gracefully) {
                (void)(sock);
                // SSL_shutdown() returns 0 on first call (indicating
                // close_notify alert sent) and 1 on subsequent call (indicating
                // close_notify alert received)
                if (SSL_shutdown(ssl) == 0) {
                        // Expected to return 1, but even if it doesn't, we free
                        // ssl
                        SSL_shutdown(ssl);
                }
        }
        ...
        while ((res = ssl_connect_or_accept(ssl)) != 1) {
                ...
                switch (err) {
                        ...
                        case SSL_ERROR_WANT_WRITE:
                                if (select_write(sock, timeout_sec,
                                                 timeout_usec) > 0) {
                                        continue;
                                }
                                break;
        ...
        if (ctx_) {
                ...
                if (SSL_CTX_use_certificate_chain_file(ctx_, cert_path) != 1 ||
                    ...
                    SSL_CTX_check_private_key(ctx_) != 1) {
                        ...
                        SSL_CTX_free(ctx_);
                        ctx_ = nullptr;
                } else if (client_ca_cert_file_path ||
                           client_ca_cert_dir_path) {
                        SSL_CTX_load_verify_locations(ctx_,
                                                      client_ca_cert_file_path,
                                                      client_ca_cert_dir_path);

                        // Set client CA list to be sent to clients during TLS
                        // handshake
                        if (client_ca_cert_file_path) {
                                auto ca_list = SSL_load_client_CA_file(
                                    client_ca_cert_file_path);
                                if (ca_list != nullptr) {
                                        SSL_CTX_set_client_CA_list(ctx_,
                                                                   ca_list);
                                } else {
                                        // Failed to load client CA list, but we
                                        // continue since
                                        // SSL_CTX_load_verify_locations already
        ...
        if (ctx_) {
                ...
                if (SSL_CTX_use_certificate(ctx_, cert) != 1 ||
                    SSL_CTX_use_PrivateKey(ctx_, private_key) != 1) {
                        ...
                        // Extract CA names from the store and set them as the
                        // client CA list
                        auto ca_list = extract_ca_names_from_x509_store(
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
        auto ssl = detail::ssl_new(
            ...
            [&](SSL* ssl2) {
                    return detail::ssl_connect_or_accept_nonblocking(
                        sock, ssl2, SSL_accept, read_timeout_sec_,
        ...
        if (ssl) {
                ...
                ret = detail::process_server_socket_ssl(
                    ...
                    keep_alive_timeout_sec_, read_timeout_sec_,
                    read_timeout_usec_, write_timeout_sec_, write_timeout_usec_,
                    [&](Stream& strm, bool close_connection,
                        bool& connection_closed) {
                            return process_request(
                                strm, remote_addr, remote_port, local_addr,
    ...
    : ClientImpl(host, port, client_cert_path, client_key_path) {
        ...
        if (!client_cert_path.empty() && !client_key_path.empty()) {
                if (!private_key_password.empty()) {
                        SSL_CTX_set_default_passwd_cb_userdata(
                            ctx_, reinterpret_cast<void*>(const_cast<char*>(
        ...
        if (ca_cert_store) {
                if (ctx_) {
                        if (SSL_CTX_get_cert_store(ctx_) != ca_cert_store) {
                                // Free memory allocated for old cert and use
                                // new store `ca_cert_store`
    ...
    Response& res, bool& success, Error& error) {
        ...
        if (!detail::process_client_socket(
                ...
                start_time, [&](Stream& strm) {
                        ...
                        req2.method = "CONNECT";
                        req2.path =
                            detail::make_host_and_port_string_always_port(
                                host_, port_);
                        if (max_timeout_msec_ > 0) {
                                req2.start_time_ =
                ...
                // Thread-safe to close everything because we are assuming there
        ...
        if (proxy_res.status == StatusCode::ProxyAuthenticationRequired_407) {
                if (!proxy_digest_auth_username_.empty() &&
                    !proxy_digest_auth_password_.empty()) {
                        std::map<std::string, std::string> auth;
                        if (detail::parse_www_authenticate(proxy_res, auth,
                                                           true)) {
                                // Close the current socket and create a new one
                                // for the authenticated request
                                shutdown_ssl(socket, true);
                                shutdown_socket(socket);
                                close_socket(socket);

                                // Create a new socket for the authenticated
                                // CONNECT request
                                ...
                                if (!detail::process_client_socket(
                                        ...
                                        start_time, [&](Stream& strm) {
                                                ...
                                                req3.method = "CONNECT";
                                                req3.path = detail::
                                                    make_host_and_port_string_always_port(
                                                        host_, port_);
                                                req3.headers.insert(
                                                    detail::make_digest_authentication_header(
                                                        req3, auth, 1,
                                        ...
                                        // Thread-safe to close everything
                                        // because we are assuming there are no
                                        // requests in flight
                                        shutdown_ssl(socket, true);
                                        shutdown_socket(socket);
                                        close_socket(socket);
            ...
            [&](SSL* ssl2) {
                    ...
                    if (server_certificate_verification_) {
                            ...
                            if (server_certificate_verifier_) {
                                    verification_status =
                                        server_certificate_verifier_(ssl2);
                            }
                            ...
                            if (verification_status ==
                                SSLVerifierResponse::NoDecisionMade) {
                                    ...
                                    auto server_cert =
                                        SSL_get1_peer_certificate(ssl2);
                                    auto se = detail::scope_exit(
...
#else
                            // NOTE: Direct call instead of using the OpenSSL
                            // macro to suppress -Wold-style-cast warning
...
inline bool SSLClient::check_host_name(const char* pattern,
        ...
        if (host_.size() == pattern_len &&
            detail::case_ignore::equal(host_,
                                       std::string(pattern, pattern_len))) {
                return true;
        }
...
inline Client::Client(const std::string& scheme_host_port,
        ...
        if (std::regex_match(scheme_host_port, m, re)) {
                ...
                // NOTE: Update TEST(UniversalClientImplTest,
                // Ipv6LiteralAddress) if port param below changes.
                cli_ = detail::make_unique<ClientImpl>(
...
inline Result Client::Post(const std::string& path, const Headers& headers,
                           ...
                           const std::string& content_type,
                           UploadProgress progress) {
        return cli_->Post(path, headers, std::move(content_provider),
                          content_type, progress);
}
inline Result Client::Post(const std::string& path, const Headers& headers,
                           ContentProviderWithoutLength content_provider,
...
inline Result Client::Put(const std::string& path, const Headers& headers,
                          ...
                          const std::string& content_type,
                          UploadProgress progress) {
        return cli_->Put(path, headers, content_length,
                         std::move(content_provider), content_type, progress);
}
inline Result Client::Put(const std::string& path, const Headers& headers,
                          size_t content_length,
...
inline Result Client::Put(const std::string& path, const Headers& headers,
                          ...
                          const std::string& content_type,
                          UploadProgress progress) {
        return cli_->Put(path, headers, std::move(content_provider),
                         content_type, progress);
}
inline Result Client::Put(const std::string& path, const Headers& headers,
                          ContentProviderWithoutLength content_provider,
...
inline ClientImpl::StreamHandle Client::open_stream(
    ...
    const Headers& headers, const std::string& body,
    const std::string& content_type) {
        return cli_->open_stream(method, path, params, headers, body,
                                 content_type);
}
...
inline long Client::get_openssl_verify_result() const {
        if (is_ssl_) {
                return static_cast<SSLClient&>(*cli_)
